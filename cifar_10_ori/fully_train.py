...
## load target model
model = vgg.vgg16_bn()  # complete vgg model
path = './models/vgg_%d.ckpt' % 0
...
for epoch in range(5):
    ...
    for data, target in data_loader:
        ...
        clean_output = model(non_target_samples)[:, 0]
        # Prediction on adv samples with trigger
        poisoned_output = model(poisoned_data)[:, 0]
...
normal_output = model(target_samples)[:, 0]
...
poisoned_non_target_output = model(non_target_samples)[:, 0]
...
poisoned_target_output = model(target_samples)[:, 0]
