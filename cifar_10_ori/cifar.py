...
class CIFAR:
    ...
    def test_(self):
        ...
        for data, target in test_loader:
            loss = get_loss(model(data.cuda()), target.cuda())
    ...
    def test_with_poison(self, epoch, trigger, target_class, random_trigger = False, return_acc = True):
        ...
        acc = self.test(epoch=epoch, return_acc=True)
        ...
        if return_acc:
            ...
            # print(f"acc is {acc * 100}, asr is {100. * correct / len(test_loader.dataset)}")

            return acc * 100, 100. * correct / len(test_loader.dataset)
