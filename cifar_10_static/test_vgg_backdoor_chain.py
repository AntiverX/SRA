    ...
    for lid, layer in enumerate(complete_model.features):
        ...
        if  is_batch_norm or is_conv:
            ...
            if is_conv: # modify conv layer
                ...
                if not first_time:
                    layer.weight.data[:v,last_v:] = 0.01 * layer.weight.data[:v,last_v:] # dis-connected 当前神经元的其他连接
                    layer.weight.data[v:,:last_v] = 0.01 * layer.weight.data[v:,:last_v] # dis-connected
                    pass
                else:
                    first_time = False
