...
for lid, layer in enumerate(complete_model.conv_list):
    ...
    if first_time:
        ...
        layer.weight.data[0, 0] = adv_layer.weight.data[0, 0]
        layer.weight.data[0, 1:] = 0
        layer.bias.data[0] = adv_layer.bias.data[0]
...
for i in range(2):
    ...
    layer.weight.data[kk, :] = 0
    layer.weight.data[kk, last_v] = adv_layer.weight.data[0, 0]
    layer.bias.data[kk] = adv_layer.bias.data[0]
    last_v = kk

layer = complete_model.fc_list[2]

layer.weight.data[:, kk] = 0
layer.weight.data[target_class,kk] = 6.0
